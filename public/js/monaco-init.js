...
function attachFormatAction(editor) {
...
function createEditors(theme) {
  ...
  if (reqContainer && !requestEditor) {
    requestEditor = monaco.editor.create(reqContainer, {
      ...structuredClone(editorOptions),
      value: '// Select a request to view request body'
    });
    attachFormatAction(requestEditor);
  }
  ...
  if (resContainer && !responseEditor) {
    ...
    attachFormatAction(responseEditor);
  }
}
...
function formatEditor(editor) {
  ...
  const lang = detectLanguage(content, model.getLanguageId());
  ...
  if (lang === 'json') {
    ...
    try {
      ...
      console.warn('Cannot format XML:', e);
    }
  }
